...
class FunctionalDependency:
    ...
    def __repr__(self):
        return str(self)
    def __hash__(self):
        return ( reduce(lambda x,y: x^y, [hash(x) for x in self.left_hand_side]) + 1 ) \
        ^ ( reduce(lambda x,y: x^y, [hash(x) for x in self.right_hand_side]) * Constants.arbitrary_prime )
